...
class BayesianOptimizer(ABC):
	...
	def check_bounds(self, bounds):
		for i, (lower, upper) in enumerate(bounds):
			if lower > upper:
				print(f"Bounds error: Lower bound ({lower}) is greater than upper bound ({upper}) at index {i}")
				return False
		return True

	def set_deep_q_network_params(self, param_names, param_values):
		if len(self.names) != len(param_values):
			raise ValueError("Length of param_names and param_values must be equal.")
		
		param_dict = dict(zip(param_names, param_values))
		
		if 'MaxMemoryLength' in param_dict:
			param_dict['MaxMemoryLength'] = int(round(param_dict['MaxMemoryLength']))
	...
	def generate_initial_data(self, n_samples, bounds, names, seed=None, save_every=None, X=True, y=True, TotalSamples=0, training_iterations=0):
		...
		for i in range(nsam):
			...
			if save_every is not None and (i+1) % save_every == 0:
				self.save_progress_to_file(self.y, n_samples - (i+1), training_iterations, False)
	...
	def maximize(self, n_iter, X, y, bounds, save_every=None, model=False):
		...
		for _ in range(n_iter):
			...
			self.y_new = self.evaluation_function(Plot=False, Start=True, OF=True)
			X = np.vstack([X, self.next_point])
			y = np.append(y, self.y_new)
			self.fit_model(X, y)
	...
	def pickup_training(self, model=True):
		...
		HP_Ranges = [value for key, value in filtered_hyperparameter_ranges.items()]
	...
	def train(self, initial_samples, training_iterations, selected_hyperparameters, RegisterTime=False, ReturnTime=False, ReturnHP=1, save_every=None, TotalSamples):
		...
		if RegisterTime:
			print(f"Start to Mid: {mid - start}, Mid to End: {end - mid}, Full Program: {end - start}")
...
class DQNAgent:
	...
	def set(self, state_size=None, action_size=None, episodes=None, batch_size=None, MaxMemoryLength=None, \
		...
		if learning_rate and learning_rate_decay:
			...
			model.set(
						optimizer=Optimizer_Adam(learning_rate=learning_rate, decay=learning_rate_decay)
					 )
	...
	def replay(self, batch_size, episode):
		...
		for state, action, reward, next_state, done in minibatch:
			...
			self.model.train(np.reshape(state, [1, state_size]), target_f, epochs=1, print_every=100)
		if self.epsilon > self.epsilon_min and self.epsilon_decay:
			self.epsilon = self.epsilon * (1 / (1 + self.epsilon_decay * episode))
		if self.learning_rate > self.learning_rate_min and self.learning_rate_decay:
			self.learning_rate = self.learning_rate * (1 / (1 + self.learning_rate_decay * episode))
	...
	def train(self, Plot=True, Start=True, OF=False, ListHP=False):
		...
		if ListHP:
			...
			print(f"batch_size: {self.batch_size:.0f}, MaxMemoryLength: 5000, gamma: {self.gamma:.2f}, Agent_Learning_Rate: {self.learning_rate:.4f},")
			print(f"Agent_Learning_Rate_Min: {self.learning_rate_min:.4f}, Agent_Learning_Rate_Decay: {self.learning_rate_decay:.4f},")
			print(f"epsilon: {self.epsilon:.4f}, epsilon_min: {self.epsilon_min:.4f}, epsilon_decay: {self.epsilon_decay:.4f},")
			print(f"learning_rate: {model.optimizer.learning_rate:.4f}, learning_rate_decay: {model.optimizer.decay:.4f}")

		if Plot or OF:
			PlotDictionary = {'ep': [], 'avg': [], 'max': [], 'min': [], 'eps': []}
			...
			for time_t in range(500):
				...
				if done:
					if (e + 1) % 200 == 0:
						...
						print(f"	episode: {e + 1}/{episodes}, score: {time_t + 1}, epsilon: {round(self.epsilon, 2)}, self LR: {round(self.learning_rate, 4)}, NN LR: {model.optimizer.learning_rate}")
		...
		if Plot:
			...
			plt.plot(PlotDictionary['ep'], PlotDictionary['eps'], label="epsilon")
			plt.legend(loc=4)
			plt.show()
...
def Handle_Hyperparameter_Ranges(path='filtered_hyperparameter_ranges.json', action=None, dictionary=False, **kwargs):
	...
	if action == 'load':
		...
		loaded_hyperparameters_tuples = [tuple(t) for t in loaded_data['tuples']]
		loaded_hyperparameters_names = loaded_data['names']
		if dictionary:
			loaded_hyperparameters_dict = {name: tuple(t) for name, t in zip(loaded_hyperparameters_names, loaded_hyperparameters_tuples)}
	...
	filtered_hyperparameters_tuples = [tuple(t) for t in filtered_hyperparameter_ranges.values()]
	filtered_hyperparameters_names = list(filtered_hyperparameter_ranges.keys())
	...
	if action == 'save':
		...
		if dictionary:
			data_to_save['dictionary'] = {name: list(t) for name, t in zip(filtered_hyperparameters_names, filtered_hyperparameters_tuples)}
	...
	if dictionary:
		filtered_hyperparameters_dict = {name: tuple(t) for name, t in zip(filtered_hyperparameters_names, filtered_hyperparameters_tuples)}
...
selected_HP = ['Agent_Learning_Rate', 'Agent_Learning_Rate_Min', 'Agent_Learning_Rate_Decay', 
							'gamma', 'epsilon', 'epsilon_min', 'epsilon_decay']

# Instantiate the required classes
model = Model()
BeyOp = ExpectedImprovement()
agent = DQNAgent()
...
# Add layers
model.add(Layer_Dense(4, 24))
model.add(Activation_ReLU())
model.add(Layer_Dense(24, 24))
model.add(Activation_ReLU())
model.add(Layer_Dense(24, 2))
...
agent.set(
			...
			Agent_Learning_Rate_Min=Agent_Learning_Rate_Min,
			Agent_Learning_Rate_Decay=Agent_Learning_Rate_Decay,
			epsilon=epsilon,
...
#HP_Names, best_hyperparameters, best_objective = BeyOp.pickup_training()
